...
int main(int argc, char* argv[]) {
	...
	while (video_capture.read(video_frame)) {
		...
		cv::Mat video_frame_;
		cv::resize(video_frame, video_frame_, cv::Size(416,416));
		input.push_back(video_frame_);
    	...
    	if (!video_frame_.empty()) {
    		...
    		std::cerr << "Error: Unable to load the image: " << video_filename << std::endl;
		}
    }
	runtime.ShutdownScheduler();
    cv::waitKey(0);
